//
//  BaseResourceController.swift
//  App
//
//  Created by Al Wold on 2/28/18.
//
...
//    }
    ...
    func show(_ req: Request, resource: T) throws ->
        ResponseRepresentable {
            let response = Response(status: .ok)
            response.resource = resource
            return response
    }
    func delete(_ req: Request, resource: T) throws ->
        ResponseRepresentable {
            try resource.delete()
    ...
    func update(_ req: Request, resource: T) throws ->
        ResponseRepresentable {
            let response = Response(status: .ok)
    ...
    func replace(_ req: Request, resource: T) throws ->
        ResponseRepresentable {
            let response = Response(status: .ok)
            guard let json = req.json else { throw Abort.badRequest }
