...
class ShoppingListControllerTests: TestCase {
    ...
    func testShoppingListCreate() throws {
        ...
        guard let listId = listJSON["id"]?.int else {
        ...
        XCTAssertEqual(listsJSON[0]["id"]?.int, listId, "Shopping List id is the same as the one created")
